...
const AT_A_GLANCE = [
  { rank:'BEST OVERALL', name:'Nfinity Vengeance', score:'9.4', price:'$110', verdict:'Buy', g:'halo' as const },
  { rank:'BEST BUDGET',  name:'Chassé Apex',       score:'8.1', price:'$60',  verdict:'Buy', g:'stripes' as const },
  { rank:'BEST FLYERS',  name:'Varsity Flyte',     score:'8.5', price:'$95',  verdict:'Buy', g:'field' as const },
  { rank:'BEST BASES',   name:'Nfinity Titan',     score:'8.7', price:'$125', verdict:'Buy', g:'glow' as const },
  { rank:'AVOID',        name:'(redacted) Pro',    score:'4.2', price:'$140', verdict:'Skip', g:'pulse' as const },
  { rank:'AVOID',        name:'Sparkle Step X',    score:'3.8', price:'$95',  verdict:'Skip', g:'block' as const },
];
...
const REVIEWS = [
  ...
  },
  {
    ...
    stars:4, g:'stripes' as const,
  },
  {
    rank:'03 · BEST FOR FLYERS', score:'8.5', name:'Varsity Flyte', price:'$95',
...
const COLS = [
  ...
  ['Varsity Flyte','6.4','high','★★★★','tight','flyers'],
...
export default function GearRoundupPage() {
  return (
    <>
      ...
      <section style={{ padding:'48px 0 56px', borderBottom:'1px solid var(--p-line)' }}>
        <Container>
          <div style={{ display:'flex', alignItems:'center', gap:14, fontSize:13, color:'var(--p-muted)', marginBottom:24 }}>
            <Link href="/">Home</Link><span>›</span><span style={{ color:'var(--p-ink)' }}>Cheer Shoes 2026</span>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr', gap:64, alignItems:'end' }}>
            <div>
              ...
              <p style={{ fontSize:19, lineHeight:1.5, color:'var(--p-inkSoft)', margin:'0 0 32px', maxWidth:600 }}>
                14 pairs. 6 months. Tested on Level 2, 4 and 5 athletes from ages 9 to 17. Here are the four worth your money — and the four that are flat-out scams.
              </p>
      ...
      <section id="picks" style={{ padding:'72px 0', background:'var(--p-paper)' }}>
        <Container>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:18, marginTop:36 }}>
            {AT_A_GLANCE.map((p, i) => (
              <div key={i} style={{ display:'grid', gridTemplateColumns:'130px 1fr', background:'var(--p-cream)', border:'1px solid var(--p-line)', cursor:'pointer' }}>
      ...
      {/* Leaderboard between picks grid and deep reviews */}
      ...
      <section style={{ padding:'80px 0', borderTop:'1px solid var(--p-line)', borderBottom:'1px solid var(--p-line)' }}>
        <Container max={1100}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:48, marginTop:48 }}>
            {REVIEWS.map((r, i) => (
              <article key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1.4fr', gap:48, padding:'32px 0', borderBottom:'1px solid var(--p-line)' }}>
                <Gradient variant={r.g} ratio="4/5" caption={r.name.toLowerCase()}/>
                <div>
                  ...
                  <div style={{ display:'flex', gap:10, marginTop:24 }}>
                    <span style={{ background:'var(--p-hot)', color:'#fff', padding:'12px 18px', fontSize:13.5, fontWeight:700, borderRadius:99, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:8 }}>Buy on Amazon <Icon.arrowUp/></span>
                    <span style={{ background:'transparent', color:'var(--p-ink)', border:'1px solid var(--p-ink)', padding:'11px 16px', fontSize:13.5, fontWeight:700, borderRadius:99, cursor:'pointer' }}>Buy direct ↗</span>
                  </div>
                </div>
              </article>
            ))}
      ...
      <section style={{ padding:'80px 0' }}>
        <Container max={1100}>
          ...
          <div style={{ marginTop:36, border:'1px solid var(--p-ink)', overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1.4fr repeat(4, 1fr)', background:'var(--p-ink)', color:'var(--p-cream)', fontSize:12, letterSpacing:'.08em', textTransform:'uppercase' as const, fontWeight:700 }}>
            ...
            {ROWS.map((r, ri) => (
              <div key={ri} style={{ display:'grid', gridTemplateColumns:'1.4fr repeat(4, 1fr)', borderTop:'1px solid var(--p-line)', fontSize:14, background: ri%2===0 ? 'var(--p-cream)' : 'var(--p-paper)' }}>
